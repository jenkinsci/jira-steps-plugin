    ...
    protected ResponseData<Object> run() throws Exception {
      ...
      if (response == null) {
        if (step.getComment() != null) {
          logger.println(
              "JIRA: Site - " + siteName + " - Add new comment (deprecated): " + step.getComment()
                  + " on issue: " + step.getIdOrKey());
          final String comment =
              step.isAuditLog() ? addPanelMeta(step.getComment()) : step.getComment();
          response = jiraService
              .addComment(step.getIdOrKey(), ImmutableMap.builder().put("body", comment).build());
        } else {
          ...
          final String message = InputBuilder.getField(step.getInput(), "body") != null
              ? InputBuilder.getField(step.getInput(), "body")
              : "";
          final String comment =
